...
def oversample(imgs, crop_size):
    """
    Crop images into the four corners, center, and their mirrored versions.
    :param imgs: iterables of (H x W x C) ndarrays
    :param crop_size: (height, width) tuple for the crops.
    :return: (10*N x H x W x K) ndarray of crops for number of inputs N.
    """
    return caffe.io.oversample(imgs, crop_size)
...
def flow_stack_oversample(flow_stack, crop_dims):
    ...
    :param flow_stack: (C x H x W)
    :param crop_dims: (10 x C x H x W)
    :return:
    """
